...
<iframe
  src="https://capture.navattic.com/clw7534lb000309mm8op8eagk?hideMobileAlert=true&scale=1"
  style={{ border: "none", width: "100%", height: "546px" }}
  allow="fullscreen" 
  id="navattic-embed"
/>
...
When adding new Members to your community, the process can be broken down into three main steps: uploading existing member data, inviting those members, or using an onboarding form link if no data is available.
<Steps>
 ...
 <Tabs>
  ...
  Enter each Member's email address individually, pressing enter after each one, or select multiple emails at a time to send a direct invitation.
   ...
   <Tab title="Choose an Invitation Method">
   **Opt-in:**
...
<Tabs>
  <Tab title="Customize the Onboarding Form">
  ...
  There are five different types of questions you can create to gather the most relevant information from your Members:
   ...
   <Accordion title="Free Response">
      Allows Members to provide detailed answers in their own words.
    </Accordion>
    <Accordion title="Single Select">
     Offers a list of options where Members can choose only one response.
    </Accordion>
    <Accordion title="Multi Select">
      Allows Members to select multiple options from a list.
    </Accordion>
    <Accordion title="Custom Scale">
      A Likert scale with custom values (e.g., "How many years of experience do you have?"). Custom scale questions must have 5 values.
    </Accordion>
    <Accordion title="Two Part Multi">
      Give & take style questions consisting of two related questions that share a similar answer bank. For example, "Where could you use support?" and "How can you support others?"
    </Accordion>
    </Tab>
    <Tab title="Copy the Onboarding Form Link">
  ...
  > Copy this link and share it with your Members via iMessage, WhatsApp, Slack, or any other platform.
    ...
    <Tab title="Preview the Onboarding Form">
 ...
 If you want to automate the process of inviting Members from other platforms like Slack or Discord, you can use Zapier to create a seamless integration. Though this method may require a bit more setup, it is definitely worth it. 
 <Tabs>
  <Tab title="Login to Zapier">
